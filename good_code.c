...
int main() {
	...
	while (!WindowShouldClose()) {
		...
		float h = piston.y - central_y;
		float vol = h * width / 1000;
		...
		DrawRectangle(piston.x, h+central_y+10, width, central_y + depth + height - piston.y-10, (Color){255,255,0,150});
		...
		bool isIsobar = (heat && !compress && !blocked);
